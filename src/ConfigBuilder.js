...
module.exports = class ConfigBuilder {
	...
	ask(questions) {
		return Ask(questions, this._options).then(answers => this._addAnswers(answers));
	}
	_addAnswers(answers) {
	...
	_add(input, cwd, level) {
		...
		if (Array.isArray(input)) return Util.asyncEachSeries(input, i => this._add(i, cwd, nextLevel));
		...
		if (typeof input === 'string') {
			...
			if (inputObject) return this._add(inputObject, cwd, nextLevel);
			const file = this._parseFile(input);

			const reader = this._options.readers[file.type || 'json'];
			if (!reader) throw new Error('Invalid type ' + file.type);
		...
		if (typeof input === 'function') {
			return Promise.resolve(input.call(null)).then(config => this._add(config, cwd, nextLevel));
		}
	...
	write(output) {
		if (this._outputConfig == null) this.process();
		return Promise.resolve(this._write(output)).then(() => this._outputConfig);
	}
	_write(output) {
		// Array, resolve every item independently
		if (Array.isArray(output)) return Util.asyncEachSeries(output, o => this._write(o));

		// String, write
		if (typeof output === 'string') {
			const file = this._parseFile(output);
			if (file.path === '-' && !file.type) file.type = 'stdout';

			const writer = this._options.writers[file.type];
			if (!writer) throw new Error('Invalid type for writing ' + file.type);
			...
			return Promise.resolve(writer.call(null, config)).then(content => fs.outputFile(filepath, content, 'utf8'));
		}
	...
	compileTemplate(template) {
		return Promise.resolve(this._compileTemplate(template)).then(() => this._config);
	}
	_compileTemplate(template) {
		// Array, resolve every item independently
		if (Array.isArray(template)) return Util.asyncEachSeries(template, t => this._compileTemplate(t));
		if (typeof template !== 'string') return;

		const templatePaths = template.match(/([^\\\:]|\\\:?)+/g).map(i => i.replace('\\:', ':'));
	...
	_resolveOutputPath(filename) {
		if (filename.substr(0, 2) === './') return filename;
	...
	_parseInputConfig(input) {
		...
		while (true) {
			...
			if (equalSignIndex < 0) return false;
			if (equalSignIndex == 0 || input.charAt(equalSignIndex - 1) !== '\\') break;
	...
	static runFromArgv(argv) {
		...
		program
			...
			.option('--env <file>', 'Environment file to load (default: .env)', collect, null)
		...
		if (program.shell) {
			return Promise.resolve().then(() => ShellOutput.print(program.shell, program.shellOption));
		}
...
function collectObject(val, memo) {
	...
	else if (value.length === 1) memo[value[0].trim()] = true;
	else if (value.length === 2) memo[value[0].trim()] = value[1].trim();
	return memo;
}
